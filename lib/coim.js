...
var coim = {
    ...
    init: function() {
        ...
        coim_timeout = coim_config.coim_timeout == undefined? 30:coim_config.coim_timeout;
        if(isNaN(coim_timeout) || coim_timeout <= 0) {
            coim_timeout = 30;
        }
        dLog("timeout: " + (coim_timeout) + " s");
    },
    ...
    send: function(relativeURL, params, success, fail, invalid){
        ...
        for(var key in _params) {
            if( Object.prototype.toString.call( _params[key] ) === '[object Array]' ) {
        ...
        var req = http.request(options, callback);
				...
				req.setTimeout(coim_timeout*1000, function(){ req.abort(); });

        /*req.on('socket', function(socket){
            socket.setTimeout(coim_timeout * 1000);
    ...
    function(relativeURL, params, files, success, fail, progress){
        ...
        function upload(req, index, files, fileFields, close) {
            ...
            fileStream.on('data', function(data) {
                ...
                if(sendData == totalSize) {
                }

                if(_success instanceof coimCallback)
                    _success.progress(sendData/totalSize * 100);
                else if(_progress !== undefined)
    ...
    getRepo: function(repoCode, callback)  {
        return  requireRepo().getRepo(repoCode, callback);
    },

    releaseRepo: function(repo)  {
        requireRepo().releaseRepo( repo );
    }
}

function  requireRepo()  {
    if (!Repo)  {
        Repo = require('./entity/Repo.js');
        Repo.setSDK( coim );
    }
    return  Repo;
}
